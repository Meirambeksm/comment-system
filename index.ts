...
// Initial variables
const list: HTMLUListElement = document.querySelector(".comment")!;
const mainListType: string = "main-list";
const addListType: string = "add-list";
...
class MainComment {
  ...
  public mainCommentEvents(): void {
    this.submitBtn?.addEventListener("click", (e: MouseEvent) =>
      this.addComment(e, this.mainInput!, list, mainListType, "0", "0")
    );
...
class PageComponents {
  ...
  constructor() {
    this.filterName = JSON.parse(
      localStorage.getItem("filterName") ?? "По актуальности"
    );
    this.responsesList = [];
  }
